...
function clip(s, n) { if (s === null || s === undefined) return null; s = '' + s; return (n > 0 && s.length > n) ? s.substring(0, n) + '..(+' + (s.length - n) + ')' : s; }
function toHex(b) {
    if (!isBytes(b)) return null;
    var n = b.length, m = (MAX_HEX > 0) ? Math.min(n, MAX_HEX) : n, s = '';
...
function toTxt(b) {
    if (!isBytes(b)) return null;
    var n = (MAX_TXT > 0) ? Math.min(b.length, MAX_TXT) : b.length, s = '';
    for (var i = 0; i < n; i++) {
        var c = b[i] & 0xff;
...
function toB64(b) {
    if (!isBytes(b)) return null;
    var n = (MAX_HEX > 0) ? Math.min(b.length, MAX_HEX) : b.length, s = '';
    for (var i = 0; i < n; i += 3) {
...
function fmtVal(x) {
    ...
    var t = typeof x;
    if (t === 'string') return { type: 'String', txt: clip(x, MAX_TXT), hex: null, b64: null };
    if (t === 'number' || t === 'boolean') return { type: t, txt: '' + x, hex: null, b64: null };
    if (isBytes(x)) return { type: 'byte[' + x.length + ']', txt: toTxt(x), hex: toHex(x), b64: toB64(x) };
    if (INSPECT_OBJECTS) {
...
function argStr(x) {
    ...
    if (INSPECT_OBJECTS) {
        var cn = safe(function () { return x.$className; }, null) || safe(function () { return '' + x.getClass().getName(); }, 'obj');
        return safe(function () { return '' + x; }, '<' + cn + '>');
    }
    return '<obj>';
}
...
function handle(entry, sig, meta, jsArgs, ret, threw) {
    ...
    rec.args = vals.length ? vals.map(function (s, k) { return 'a' + k + '=' + (s === null ? 'null' : s); }).join(' | ') : '()';

    /* 返回（或异常），不截断 */
    var rv = threw ? { type: 'throw', txt: '' + threw, hex: null, b64: null } : fmtVal(ret);
    rec.ret_type = rv.type; rec.ret_txt = rv.txt; rec.ret_hex = rv.hex; rec.ret_b64 = rv.b64;
    ...
    if (!vals.length) line += '\n   (无参)';
    for (var j = 0; j < vals.length; j++) line += '\n   arg' + j + ' = ' + (vals[j] === null ? 'null' : vals[j]);
    line += '\n   ret  = ' + (rv.txt === null ? 'null' : rv.txt) + (rv.hex ? '  hex=' + rv.hex : '') + (rv.b64 ? '  b64=' + rv.b64 : '') + '  (' + rv.type + ')';
    if (rec.stack) line += '\n' + rec.stack;
    clog(line);

...
function clip(s) { s = '' + s; return s.length > MAX_TXT ? s.slice(0, MAX_TXT) + '..(+' + (s.length - MAX_TXT) + ')' : s; }
function hitOf(v) {
  if (v === null || v === undefined) return null;
  var s = '' + v;
  for (var i = 0; i < TARGETS.length; i++) if (TARGETS[i] && s.indexOf(TARGETS[i]) !== -1) return TARGETS[i];
  return null;
}

Java.perform(function () {
  if (!TARGETS.length) { console.log('[trace-src] 先填 TARGETS（你的 device_md），否则没目标可追。'); return; }
  ...
  function report(where, key, value, ctx) {
    ...
    console.log(stk);
    console.log('################################################\n');
    try { send({ type: 'sign', data: { kind: 'SRC@' + where, input_txt: '' + key, out_b64: '' + value, stack: stk, matched: 1 } }); } catch (e) {}
  }
  ...
  function hookReturn(cls, names, keyIdx, withCtx) {
    ...
    if (!clazz) { console.log('[trace-src] 跳过 ' + cls + '（未解析，可能没用它或类名被改）'); return; }
    names.forEach(function (mn) {
      ...
      m.overloads.forEach(function (ov) {
        ov.implementation = function () {
          ...
          try {
            if (hitOf(r)) {
              var key = (keyIdx != null && arguments.length > keyIdx) ? arguments[keyIdx] : '?';
              var ctx = withCtx ? safe(function () { return '' + self.toString(); }, null) : null;
      ...
      console.log('[trace-src] hooked ' + cls + '.' + mn + ' x' + m.overloads.length);
    });
  }
  ...
  if (Ed && Ed.putString) Ed.putString.overloads.forEach(function (ov) {
    ov.implementation = function (k, v) { try { if (hitOf(v)) report('SP.putString', k, v, null); } catch (e) {} return ov.apply(this, arguments); };
  });
  ...
  console.log('\n[trace-src] 就位。盯：' + JSON.stringify(TARGETS.map(function (t) { return (t || '').slice(0, 8) + '..'; })));
  console.log('[trace-src] 触发设备刷新看读取点；退出账号 -> 重新登录 看下发/生成点。');
  console.log('[trace-src] 全都没命中 = 值可能常驻内存（登录时算好放单例字段）或被别的存储持有，转静态 jadx 看 OkHttpRequest(vc.d).a。\n');
});

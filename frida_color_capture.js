...
Java.perform(function () {
    ...
    console.log('\n[*] 彩虹网关抓包+sign+加密信封追踪+jdupgrade d.a(HmacSHA256) 已启动。');
    console.log('[*] 触发一次 getHouses 后，SQL 对一下 d.a 输出与 wire sign：');
    console.log("    SELECT s.id,s.out_hex,c.sign,c.t FROM sign s JOIN color c ON lower(s.out_hex)=lower(c.sign) WHERE s.kind='HMAC.a' ORDER BY s.id DESC;\n");
});

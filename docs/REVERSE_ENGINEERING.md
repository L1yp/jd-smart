...
| [`frida_color_capture.js`](../frida_color_capture.js) | **彩虹网关版**（api.m.jd.com）：okhttp 抓包 + SHA-256 sign + ep/body 加密信封追踪 + **Part7 jdupgrade `d.a`(HmacSHA256) sign 嫌疑**（落 `color`/`cipher`/`sign` 表，`HMAC.*`，含 `rpc.exports.dahmac`，见 §8/§8.7） |

...
- `sign` 是 **32 字节**（64 hex），直接排除 MD5（16B），锁定 **SHA-256 / HmacSHA256**——与旧接口
  ...
  **已收口（§8.7-5 / 进度 §7.3）**：`sign = HMAC-SHA256(preimage, secret)`，原语 = native `NativeAlgorithmHelper.getHmacSha256Value`，
  `secret` 固定内嵌（`getSecretKey()`），key 当 32 字符 ASCII 文本；preimage = TreeMap 各 value 按序 `&` 拼。
...
**`ciphertype:5` 实测结论：就是「自定义字母表的标准 Base64」——不是加密，是换表混淆（已解）。**

- 解码算法（逆向自 App `decode()`）= 标准 base64 位拼装（大端 6bit），只是字符→6bit 用一张**打乱的
  64 字符表**：`KLMNOPQRST ABCDEFGHIJ UVWXYZ abcd opqrstuvwx efghijklmn yz 0-9 +/`。
- ⇒ **可完全离线 decode/encode，无需 hook**。实现见 [`color_codec.py`](../color_codec.py)（含自测 + 往返校验）。
- 实测解出（往返全通过）：`client=android`、`networkType=wifi`、`d_brand=HUAWEI`、`d_model=HWI-AL00`、
  `screen=1080*2160`、`osVersion=28`、`clientVersion=1.17.0`、`partner=xjgw-android`、`build=381`、
  `ext={"prstate":"0"}`、`eid=<同 cookie devicefinger>`、`aid=<同 query uuid>`；
  **`body.cipher.body` = `{"pageSize":100,"page":1}`**（getHouses 真实请求体，就是分页）。

> 早期「必须 live 抓加密函数」的判断**作废**——那是误把换表 base64 当成自有加密；`ep`/`body` 现已可离线自造。
> 连带：§8.7 里「`LoadDoor.enc/dec` 可能就是 ciphertype:5」的猜测也排除（ciphertype:5 是纯 Java 换表 base64）；
> LoadDoor 仍可能与 `sign`/`getToken` 相关。
...
2. **第二遍（钉死）**：把定位到的类全名填进脚本顶部 `ENCRYPT_CLASSES`，脚本自动 hook 其所有
   `String->String` 方法，抓**明文↔密文**。
   > 注：`ciphertype:5` 已证实是换表 base64（§8.3 / [`color_codec.py`](../color_codec.py)），**ep/body 这步已不需要**；
   > 仅当出现别的 `ciphertype` 或别处真加密时才用第二遍。
...
3. **算法已逆（源码确认）**：`com.jingdong.sdk.jdupgrade.inner.c.a(functionId,query,body)` 调
   `utils.d.a(data, key)`：`sign = HmacSHA256(data, key)→64hex`；`data` = `TreeMap(比较器 b){functionId+query+body}`
   的**各 value** 按 key 序用 `&` 拼（只拼 value、不含 key、去尾随 &）、`body=modBase64(gzip(json))`；`key` = 固定
   secret（`c.W()` 选 prod/test，**是 32 字符 hex 文本的 UTF-8 字节(32B)，非 fromhex 的 16B**）。
   ⚠ 这是 **jdupgrade（升级）签名器**，getHouses body 是 `ciphertype:5`（≠ gzip+modB64），很可能另有并行签名器、
   共用 `d.a`。`frida_color_capture.js` **Part 7** 已挂 `d.a`（`HMAC.a`，data→input_*/key→key_*）；与 okhttp 同跑后对：
   `SELECT s.out_hex,c.sign FROM sign s JOIN color c ON lower(s.out_hex)=lower(c.sign) WHERE s.kind='HMAC.a';`
   命中 ⇒ 复用，该 `input_txt` 直接就是 getHouses preimage（免逆 b）。离线核对 [`verify_color_sign.py`](../verify_color_sign.py)；
   现算 REPL `rpc.exports.dahmac('<新 t preimage>','<secret>')`（顺序=d.a：先 data 后 key）。
4. **实测落点（真签名器）**：连发 getHouses 后 `HMAC.a` 不命中 ⇒ **getHouses 不走 jdupgrade `d.a`**（仅升级用）。真签名器在
   `com.jd.smart.networklib`：Java `SignRequestInterceptor.c(String)`（`SIGNI.c`）+ native
   `NativeEncodeDataToServer(String,long,String×5,int)`/`_gm`（国密，`NSIGN.*`）。`frida_color_capture.js` **Part 8** 已挂
   （native 类名留空自动发现），同跑后对：
   `SELECT s.kind,s.out_hex,c.sign FROM sign s JOIN color c ON lower(s.out_hex)=lower(c.sign) WHERE s.kind LIKE 'NSIGN.%' OR s.kind LIKE 'SIGNI.%';`
   命中即真签名器，其 `input_txt`（native 8 入参/拦截器入参）就是被签料；若对的是 `c.body_cipher` 则该 native 是「整体加密 data」。
5. **HMAC 原语锁定 = `com.jd.smart.algorithm.NativeAlgorithmHelper`（native，`libnativealgorithm.so`）**：
   `getHmacSha256Value(String data)`（1 参）/ `(String key,String data)`（2 参）/ `getSecretKey()`。App 实际**只命中 1 参**版 →
   key 来自 native 内部（`getSecretKey()`），故 **secret 固定**（按构建内嵌，非每请求传入；同机 replay 只 `t` 变、key 不变）。
   **算法 = 标准 HMAC-SHA256，key 当 32 字符 ASCII 文本(32B)、非 fromhex 16B**——已用抓包库一条完整三元组（id 373–375，`stats-api`
   子系统同款约定）离线证死：`hmac(key.encode(), data.encode(), sha256).hexdigest()` == 抓到的 `out_hex`；`bytes.fromhex(key)` 形不命中。
   注：getAllDevices/getHouses 走 native 这套，绕开 `javax.crypto.Mac`，所以其密钥 `6b086ed2…` **不出现**在 `sign` 表 `SecretKeySpec/Mac.*` 行
   （那里只有 stats-api/mpaas2 的 `121dce72…`/`f947bd59…`/`e45e21c9…`）。preimage = TreeMap 各 value 按序 `&` 拼。
   抓 (data→sign)+`getSecretKey()` 用 [`frida_sign_pipeline_capture.js`](../frida_sign_pipeline_capture.js)；离线核对
   `python verify_color_sign.py --preimage "<data>" --key <getSecretKey> --expect <sign>`（详见进度 §7.3）。
...
| [`frida_color_capture.js`](../frida_color_capture.js) | **彩虹网关版**（api.m.jd.com）：okhttp 抓包 + SHA-256 sign + ep/body 加密信封 + **Part7 jdupgrade `d.a`(HmacSHA256，仅升级)** + **Part8 真签名器 `SignRequestInterceptor.c`/native `NativeEncodeDataToServer(_gm)`**（落 `color`/`cipher`/`sign` 表，`HMAC.*`/`SIGNI.*`/`NSIGN.*`，含 `rpc.exports.dahmac`，见 §8/§8.7） |
...
| [`frida_sign_pipeline_capture.js`](../frida_sign_pipeline_capture.js) | **签名流水线三层**（纯 console，跨 ClassLoader）：A `NativeAlgorithmHelper.getSecretKey()`/`getHmacSha256Value`（真 HMAC 原语，§8.7-5 / 进度 §7.3）+ B `light_http_toolkit.util.c.a(map,sign)` 贴 sign + C `manto.network.common.a.b` 小程序下发；全局 #N 串因果 |
| [`frida_mobileconfig_sign_capture.js`](../frida_mobileconfig_sign_capture.js) | **mobileConfig 签名**（纯 console）：hook `d.b` 拼接(String×3) + 计算(byte[],byte[]) 的 sign |
| [`frida_cryptoutils_capture.js`](../frida_cryptoutils_capture.js) | **`com.wangyin.platform.CryptoUtils` 全方法**（国密/加解密原语）入出（落 `crypto_utils` 表） |
...
| [`verify_sign.py`](../verify_sign.py) | 离线一比一复现/校验旧接口签名（HmacSHA1） |
| [`verify_color_sign.py`](../verify_color_sign.py) | 离线复现彩虹/jdupgrade 的 `HmacSHA256(preimage,secret)→64hex`（核对 wire sign / 换新 t 重算；密钥读 gitignored `jd_smart_secrets.json`，见 §8.7 / 进度 §7.1） |
| [`color_sign.py`](../color_sign.py) | **彩虹 query sign 生成器**：18 键**字母序**拼 preimage + HMAC-SHA256（key 当文本）；`--from-db` 从最新 `ep` 自动解 14 项设备档，每请求只换 functionId/body/t（密钥+设备档读 gitignored `jd_smart_secrets.json` 的 `color_sign_secret`/`color_profile`，见进度 §7.3） |
...
| [`docs/GETHOUSES_PROGRESS.md`](GETHOUSES_PROGRESS.md) | **getHouses 进度归档**：可重放性矩阵 + 已确认来源 + sign/ep 下一步 runbook |
| [`color_codec.py`](../color_codec.py) | **`ciphertype:5` 编解码器**（自定义字母表 base64）：离线 decode/encode `ep`/`body`，含自测 |

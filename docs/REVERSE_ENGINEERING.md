...
3. **直挂头号嫌疑 `d.a`**：静态分析锁定 `com.jingdong.sdk.jdupgrade.inner.utils.d.a(byte[],byte[])` 内部为
   **HmacSHA256**（32B，与 sign 同型）。`frida_color_capture.js` **Part 7** 已挂它（落 `sign` 表 `kind=HMAC.a`，
   抓两入参+输出+栈），与 okhttp 同跑后 SQL 直接对：
   `SELECT s.out_hex,c.sign FROM sign s JOIN color c ON lower(s.out_hex)=lower(c.sign) WHERE s.kind='HMAC.a';`
   命中即 d.a 为 sign，其 data 入参 = preimage、另一入参 = key（key 同时被 `[Mac.init]` 打出可交叉确认）。
   REPL `rpc.exports.dahmac('hex:<keyhex>','<新 t preimage>')` 用 App 自带实现现算，验证「新 t ⇒ 新 sign」。
